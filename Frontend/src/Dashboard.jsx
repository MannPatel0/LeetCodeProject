  ...
  const Dashboard = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_GETURL}${username}`
        );
    ...
    const fetchProblems = async () => {
      try {
        //
        const response = await fetch(import.meta.env.VITE_GETURL);
        const data = await response.json();
        setProblems(data);
      } catch (error) {
      ...
      <div className={`min-h-screen ${theme.background} ${theme.text}`}>
        <nav className={`border-b ${theme.border} p-4`}>
          <div className="container mx-auto flex justify-between items-center">
            <h1 className="text-xl font-bold">LeetCode Pattern</h1>
                    ...
                    <tr key={index} className={`border-t ${theme.border}`}>
                      <td className="p-4">
                        <a
                          ...
                          className={`${theme.link} text-white hover:text-blue-500`}
                        >
                          {problem.title}
                        </a>
                      </td>
        ...
        >
          <p>LeetCode Pattern MIT Licenced</p>
          <p>
            Created with the help from the following sources:
            <a
            ...
            , <a
              href="https://www.github.com/mannpatel0"
              className={theme.link}
            >
              {" "}
              Mann Patel
